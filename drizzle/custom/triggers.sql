...
-- Applied manually after drizzle-kit push (not managed by Drizzle)
-- =============================================================================
...
DECLARE
  t TEXT;
  tables TEXT[] := ARRAY[
    'users', 'applications', 'documents', 'rooms', 'room_allocations',
    'fees', 'payments', 'leave_requests', 'renewals', 'gateway_payments',
    'leave_types', 'blackout_dates', 'notification_rules', 'interviews',
...
-- 2. Validate application status transitions
-- =============================================================================
...
BEGIN
  ...
  allowed := valid_transitions -> OLD.current_status;

  IF allowed IS NULL OR NOT allowed ? NEW.current_status THEN
  ...
  CASE NEW.current_status
    ...
    WHEN 'REVIEW' THEN NEW.reviewed_at := NOW();
    WHEN 'INTERVIEW' THEN NEW.interview_scheduled_at := NOW();
...
-- 3. Update room occupancy on allocation changes
-- =============================================================================
  ...
  IF TG_OP = 'INSERT' AND NEW.status = 'ACTIVE' THEN
    UPDATE rooms SET
      ...
      status = CASE
        ...
        WHEN occupied_count + 1 > 0 THEN 'OCCUPIED'
        ELSE status
      END
    WHERE id = NEW.room_id;
  ELSIF TG_OP = 'UPDATE' THEN
    -- Deactivating an allocation
    IF OLD.status = 'ACTIVE' AND NEW.status != 'ACTIVE' THEN
      UPDATE rooms SET
        occupied_count = GREATEST(occupied_count - 1, 0),
        status = CASE
...
-- 4. Validate leave status transitions
-- =============================================================================
...
CREATE TRIGGER trg_validate_leave_status
  BEFORE UPDATE ON leave_requests
  FOR EACH ROW
  WHEN (OLD.status IS DISTINCT FROM NEW.status)
  EXECUTE FUNCTION validate_leave_status_transition();

-- 5. Auto-update fee status when payment is recorded
-- =============================================================================
...
BEGIN
  IF NEW.status = 'PAID' AND NEW.fee_id IS NOT NULL THEN
    UPDATE fees SET
      status = 'PAID',
      paid_at = NOW()
    WHERE id = NEW.fee_id AND status != 'PAID';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- 6. Auto-generate tracking number for applications
-- =============================================================================
...
BEGIN
  ...
  CASE NEW.vertical
    WHEN 'BOYS' THEN prefix := 'BH';
    WHEN 'BOYS_HOSTEL' THEN prefix := 'BH';
    WHEN 'GIRLS' THEN prefix := 'GA';
    WHEN 'GIRLS_ASHRAM' THEN prefix := 'GA';
    WHEN 'DHARAMSHALA' THEN prefix := 'DS';
    ELSE prefix := 'HP';
  END CASE;
  ...
  SELECT COALESCE(MAX(
    CASE
      WHEN tracking_number ~ (prefix || '-' || year_str || '-[0-9]+')
      THEN CAST(SUBSTRING(tracking_number FROM (prefix || '-' || year_str || '-([0-9]+)')) AS INTEGER)
      ELSE 0
    END
  ), 0) + 1
  INTO seq_num
...
-- 7. Prevent audit log modification (immutable)
-- =============================================================================
CREATE OR REPLACE FUNCTION prevent_audit_log_modification()
RETURNS TRIGGER AS $$
BEGIN
  RAISE EXCEPTION 'Audit logs are immutable — cannot % audit_logs', TG_OP;
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;
