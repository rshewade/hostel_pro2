...
export async function GET(req: NextRequest) {
  try {
    ...
    const studentId = session.user.role === 'STUDENT' ? userId : undefined;
    const result = await listLeaves({ studentUserId: studentId, status, page: params.page, limit: params.limit });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const leave = await createLeaveRequest(body, userId);
