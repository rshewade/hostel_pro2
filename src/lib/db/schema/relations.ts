...
export const documentsRelations = relations(documents, ({ one }) => ({
  ...
  studentUser: one(users, { fields: [documents.studentUserId], references: [users.id] }),
}));
...
export const roomAllocationsRelations = relations(roomAllocations, ({ one }) => ({
  ...
  studentUser: one(users, { fields: [roomAllocations.studentUserId], references: [users.id] }),
}));

export const feesRelations = relations(fees, ({ one, many }) => ({
  studentUser: one(users, { fields: [fees.studentUserId], references: [users.id] }),
...
export const paymentsRelations = relations(payments, ({ one }) => ({
  ...
  studentUser: one(users, { fields: [payments.studentUserId], references: [users.id] }),
}));

export const leaveRequestsRelations = relations(leaveRequests, ({ one }) => ({
  studentUser: one(users, { fields: [leaveRequests.studentUserId], references: [users.id] }),
}));

export const renewalsRelations = relations(renewals, ({ one }) => ({
  studentUser: one(users, { fields: [renewals.studentUserId], references: [users.id] }),
...
export const gatewayPaymentsRelations = relations(gatewayPayments, ({ one }) => ({
  studentUser: one(users, { fields: [gatewayPayments.studentUserId], references: [users.id] }),
  application: one(applications, { fields: [gatewayPayments.applicationId], references: [applications.id] }),
  fee: one(fees, { fields: [gatewayPayments.feeId], references: [fees.id] }),
}));
