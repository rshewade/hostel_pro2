...
export const gatewayPayments = pgTable('gateway_payments', {
  ...
  index('idx_gateway_payments_application_id').on(table.applicationId),
  index('idx_gateway_payments_fee_id').on(table.feeId),
  index('idx_gateway_payments_razorpay_order_id').on(table.razorpayOrderId),
  index('idx_gateway_payments_razorpay_payment_id').on(table.razorpayPaymentId),
  index('idx_gateway_payments_status').on(table.status),
  index('idx_gateway_payments_idempotency_key').on(table.idempotencyKey),
  index('idx_gateway_payments_paid_at').on(table.paidAt),
]);
